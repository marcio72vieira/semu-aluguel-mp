            ...
            <table class="table table-striped table-hover">
                <thead>
                    <tr>
                        ...
                        <th>Nome</th>
                        <th>Usuário</th>
                        <th class="d-none d-md-table-cell">Regional / Município</th>
                        <th class="d-none d-md-table-cell">Unidade de Atendimento</th>
                ...
                <tbody>
                    ...
                    @forelse ($users as $user)
                        <tr>
                            <td>{{ $user->id }}</th>
                            <td>{{ $user->tipounidade->nome }}</th>
                            <td>{{ $user->nome }}</td>
                            <td>{{ $user->regional->nome }}</td>
                            ...
                            <td class="flex-row d-md-flex justify-content-start">
                                ...
                                @if(2 > 1)
                                    <form id="formDelete{{ $user->id }}" method="POST" action="{{ route('user.destroy', ['user' => $unidadeatendimento->id]) }}">
                                        @csrf
                                        @method('delete')
