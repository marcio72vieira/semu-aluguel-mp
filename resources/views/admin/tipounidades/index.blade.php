                        ...
                        <tr>
                            ...
                            <td class="flex-row d-md-flex justify-content-start">
                                ...
                                @if($tipounidade->qtdunidadesatendimento($tipounidade->id) == 0)
                                    <form id="formDelete{{ $tipounidade->id }}"  method="POST" action="{{ route('tipounidade.destroy', ['tipounidade' => $tipounidade->id]) }}">
                                        @csrf
                                        @method('delete')
                                        <button type="submit" class="mb-1 btn btn-danger btn-sm me-1  btnDelete"  data-delete-entidade="Tipo de Unidade" data-delete-id="{{ $tipounidade->id }}"  data-value-record="{{ $tipounidade->nome }}">
